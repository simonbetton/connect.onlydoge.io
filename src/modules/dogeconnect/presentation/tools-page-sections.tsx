...
export function ToolsPageHero() {
  return (
    <PageHero
      ...
      aside={
        <>
          <Badge variant="neutral">strict validation</Badge>
          <Badge variant="neutral">simulator relay</Badge>
        </>
      }
    >
      <div className="flex flex-wrap gap-2">
        <Link to="/flight-recorder" className={buttonVariants({ variant: "outline", size: "sm" })}>
      ...
      {generateMockQrMutation.data ? (
        <div className="min-w-0 space-y-3 rounded-2xl border border-border/70 bg-background/60 p-4">
          <div className="grid gap-2 text-xs sm:grid-cols-3">
            ...
            <p className="truncate text-muted-foreground">
              dc:{" "}
              <span className="font-medium text-foreground">{generateMockQrMutation.data.dc}</span>
            </p>
          </div>
          <Textarea
