...
export const createHttpFlightRecorderEnvelopeClient = (): TracedEnvelopeClientPort => ({
  fetchEnvelope: (input) => fetchRemoteEnvelope(input),
})

const fetchRemoteEnvelope = async (input: {
  connectUrl: string
}): Promise<TracedEnvelopeFetchResult> => {
