...
import { DogeUri, isDogeConnectUri } from "./doge-uri"
...
describe("DogeUri", () => {
  test("parses a valid DogeConnect URI", () => {
    ...
    expect(result.value && isDogeConnectUri(result.value)).toBe(true)
  ...
  test("keeps plain dogecoin URI as non-connect informational case", () => {
    ...
    const result = DogeUri.parse(uri)
    expect(result.value && isDogeConnectUri(result.value)).toBe(false)
    expect(result.issues.filter((issue) => issue.severity === "error")).toHaveLength(0)
  })
